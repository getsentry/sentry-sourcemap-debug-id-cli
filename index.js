...
yargs(hideBin(process.argv))
  .command(
    ...
    async ({ targets }) => {
      const normalizedAbsolutePaths = targets.map((target) =>
        path.resolve(target)
      );
      ...
      await Promise.all(
        filePaths.map(async (filePath) => {
          ...
          const sourceMappingURLMatch = jsFileData.match(
            /^\/\/# sourceMappingURL=(.*)$/m
          );
          ...
          if (jsFileData.includes("//# sentryDebugId=")) {
            // eslint-disable-next-line no-console
            console.log(
              `Warning: File "${filePath}" was already processed. Will skip processing for this file.`
            );
            return;
          }
          ...
          const normalizedSourceMapURL = path.normalize(
            sourceMappingURLMatch[1]
          );
          ...
          try {
            ...
            // eslint-disable-next-line no-console
            console.log(
              `Warning: Failed to parse source map ${sourceMapPath}. Will skip processing for this file.`
            );
            return;
          }

          const debugID = uuidv4();
          const codeToInject = `${INJECTOR_CODE}${DEBUG_ID_COMMENT}`.replace(
          ...
          return await Promise.all([
            writeFile(sourceMapPath, JSON.stringify(sourceMap), {
              encoding: "utf8",
              flag: "w",
            }), // w flag is for overriding existing file
...
async function getFilesFromAbsolutePath(absolutePath) {
  ...
  if (stats.isFile()) {
    ...
    const results = await Promise.all(
      files.map((file) =>
        getFilesFromAbsolutePath(path.join(absolutePath, file))
      )
    );
